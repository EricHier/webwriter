...
async function generateManifest() {
    ...
    // Path of cem.config.mjs in our package
    const selfPath = new URL(import.meta.url).pathname.substring(1);
    const cemConfigPath = join(selfPath, "../cem.config.mjs")

    // Add widget paths to cem config
    let cemConfig = await fs.readFile(cemConfigPath, "utf8")

    // Replace globs line
    const globsLine = `  globs: [${widgetPaths.map(p => `\`${p}\``).join(", ")}],`
    cemConfig = cemConfig.replace(/  globs: \[.*\],/s, globsLine)

    // Write into temporary cem config in cwd
    const tempCemConfigPath = join(process.cwd(), "cem.config.mjs")
    await fs.writeFile(tempCemConfigPath, cemConfig, "utf8")

    // Run analyzer with that config
    console.log("Running CEM analyzer...")

    try {
        const {
            stdout,
            stderr
        } = await exec(`npx @custom-elements-manifest/analyzer analyze --config ${tempCemConfigPath}`)
        stderr ? console.error(stderr) : console.log(stdout)
    } catch (err) {
        console.error("Error running CEM analyzer:", err?.message ?? String(err))
    }

    // Remove temporary cem config
    await fs.unlink(tempCemConfigPath)

    // Add customElements field to package.json if not present
