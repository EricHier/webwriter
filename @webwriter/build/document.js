...
async function generateManifest() {
    ...
    // Resolve the path to the inheritance plugin relative to this script
    const currentDir = dirname(fileURLToPath(import.meta.url));
    const inheritancePluginPath = join(currentDir, '..', '..', 'node_modules', 'custom-elements-manifest-inheritance', 'index.js');


    // Create CEM analyzer config file
    const cemConfig = `import fs from 'fs';
import { pathToFileURL } from 'url';

export default {
  /** Globs to analyze */
  globs: ${JSON.stringify(widgetPaths)},
  ...
  plugins: [
    {
      name: 'inheritance-plugin',
      async packageLinkPhase({ customElementsManifest, context }) {
        const { processInheritance } = await import(pathToFileURL('${inheritancePluginPath.replaceAll("\\", "/")}').href);
        processInheritance(customElementsManifest, context);
      }
    },
    // Plugin to extract full JSDoc descriptions by parsing source manually
    {
      ...
      ${widgetPaths.map(widgetPath => `
        try {
          
          const sourceContent = fs.readFileSync('${widgetPath}', 'utf-8');
          
          // Extract the main class JSDoc comment manually
          const classCommentMatch = sourceContent.match(/\\/\\*\\*([\\s\\S]*?)\\*\\/[\\s]*@customElement[\\s\\S]*?export default class \\S+/);
    ...
    await fs.writeFile("./cem.config.mjs", cemConfig, "utf8")

    const { stdout, stderr } = await exec(`npx @custom-elements-manifest/analyzer analyze --config ./cem.config.mjs`)
    stderr ? console.error(stderr) : console.log(stdout)

    // Clean up config file
    // await fs.unlink("./cem.config.mjs").catch(() => {});
